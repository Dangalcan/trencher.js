...
const {
  handleValidation,
} = require('../middlewares/TrencherValidationHandlingMiddleware');
...
function configureMulter({ folder = 'public/uploads', fileProperties = [] }) {
  const storage = multer.diskStorage({
    ...
    filename: function (req, file, cb) {
      cb(
        null,
        `${Math.random().toString(36).substring(7)}-${Date.now()}.${file.originalname.split('.').pop()}`,
      );
    },
  });
  ...
  const uploadFields =
    fileProperties.length > 0
      ? fileProperties.map((prop) => ({ name: prop }))
      : [];
...
function getAllRoute(app, basePath, getAllMethod, middlewares = []) {
  app.route(basePath).get(...middlewares, getAllMethod);
}
...
function getRoute(
  ...
  app.route(`${basePath}/:${idPropertyName}`).get(...middlewares, getMethod);
}
...
function updateRoute(
  ...
  app.route(`${basePath}/:${idPropertyName}`).put(
    ...middlewares,
    (req, res, next) => {
...
function patchRoute(
  ...
  app.route(`${basePath}/:${idPropertyName}`).patch(
    ...middlewares,
    (req, res, next) => {
      const uploader = configureMulter(multerOptions);
      uploader(req, res, (err) => {
...
function deleteRoute(
  ...
  app
    .route(`${basePath}/:${idPropertyName}`)
    .delete(...middlewares, deleteMethod);
}
...
function CRUDRoutes(
  ...
  multerOptions = {},
  controller,
  specificGetAllMiddlewares = [],
  specificCreateMiddlewares = [],
  specificUpdateMiddlewares = [],
  specificPatchMiddlewares = [],
  specificGetMiddlewares = [],
  specificDeleteMiddlewares = [],
) {
  const {
    getAllMethod,
    getMethod,
...
function customGETRoute(app, path, controllerMethod, middlewares = []) {
  app.route(path).get(...middlewares, controllerMethod);
}
...
function customDELETERoute(app, path, controllerMethod, middlewares = []) {
  app.route(path).delete(...middlewares, controllerMethod);
}
