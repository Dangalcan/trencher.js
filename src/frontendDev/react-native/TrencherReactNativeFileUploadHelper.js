...
const prepareEntityImages = (entity, imagePropertyNames) => {
  ...
  imagePropertyNames.forEach((imagePropertyName) => {
    if (entityCopy[imagePropertyName]) {
      entityCopy[imagePropertyName] = {
        assets: [
          {
            uri: `${process.env.API_BASE_URL}/${entityCopy[imagePropertyName]}`,
          },
        ],
      };
    }
...
const normalizePDFFile = (file, paramName) => {
  if (Platform.OS === 'web') {
    ...
    if (file.uri.startsWith('data:')) {
      ...
      throw new InternalTrencherError(
        'This file format is not supported by PDF.',
      );
    }
...
function constructFormData(files, dataWithoutFiles) {
  ...
  files.forEach((file) => {
    ...
    if (
      ...
      mime.lookup(asset.uri) === 'application/pdf'
    ) {
