...
function getKeyAndIv(){
    ...
    if(!process.env.ENCRIPTER_KEY || !process.env.ENCRIPTER_IV){
        logger.warn('There is no encripting key and no inizialization vector. System is generating them')
...
function decryptPropertiesWithEnvValues(encryptedObject, propertiesToDecrypt) {
    ...
    const key = process.env.ENCRIPTER_KEY ? forge.util.decode64(process.env.ENCRIPTER_KEY) : envKey
    const iv = process.env.ENCRIPTER_IV ? forge.util.decode64(process.env.ENCRIPTER_IV) : envIv
    envKey = null
    envIv = null
 ...
 * @author Daniel Galván Cancio
 */
function decryptPropertiesWithRamdomKey(encryptedObject, keyBase64, ivBase64, propertiesToDecrypt) {
...
const encryptedObject3 = encryptPropertiesWithEnvValues(object3, propertiesToEncrypt2);
console.log('Encrypted Object with env values:', encryptedObject3);

const decryptedObject3 = decryptPropertiesWithEnvValues(encryptedObject3,propertiesToEncrypt2);
...
const encryptedObject = encryptPropertiesWithRamdomKey(object, propertiesToEncrypt2);
console.log('Encrypted Object with ramdom values:', encryptedObject);

const decryptedObject = decryptPropertiesWithRamdomKey(encryptedObject.encryptedObject, encryptedObject.key, encryptedObject.iv,propertiesToEncrypt2);
